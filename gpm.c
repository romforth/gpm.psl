/*
 * C implementation of Christopher Strachey's GPM (Gemeral Purpose Macrogenerator)
 * converted from the rust version at https://github.com/psl8/gpm
 * which in turn was converted from the CPL version in Strachey's '68 paper
 *
 * Rust to C conversion was mostly done using a brute force line level "translator"
 * written in Perl which worked for ~80% of the code - the rest of the heavy lifting
 * was done by hand.
 *
 * Copyright (c) 2026 Charles E. Suresh, Patty S. Loughney, Christopher J. Strachey
 *
 * Licence: AGPLv3
...
// n is stack size allowed. This should be as large as possible -- say 10,000.
mem *
gpm_new(int n) {
	mem *gpm = malloc(n);
	if (!gpm) {
